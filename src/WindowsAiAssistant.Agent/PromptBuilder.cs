...
public sealed class PromptBuilder
{
        ...
        string? triggerSource = null)
    {
        ...
        if (string.Equals(triggerSource, "chat", StringComparison.OrdinalIgnoreCase))
        {
            ...
            builder.AppendLine("- Desktop automation in OTHER applications is allowed. If activeProcessName is WindowsAiAssistant, call focus_window to the named app (e.g. Cursor, Word) before type_text/click.");
            builder.AppendLine("- Multi-step goals (web search + write to document) need efficient steps: shell/curl or clipboard for long text, not many small type_text calls.");
