...
public sealed class GeminiModelDecisionProvider : IModelDecisionProvider
{
    ...
    public GeminiModelDecisionProvider(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_profile.Kind != ModelProviderKind.Gemini)
        {
        ...
        try
        {
            ...
            if (!TryExtractGeminiTextContent(responseBody, out var messageContent, out var extractReason))
            {
                var extractDiagnostic = extractReason ?? "missing-model-text";
                return CreateInvalidResponseResult(extractDiagnostic);
            }
        ...
        {
            return CreateNotAvailableResult("timeout");
        }
    }
        ...
        var payload = new
        {
            ...
            contents = new object[]
            {
                new
                {
                    ...
                    parts = new object[]
                    {
                        new { text = BuildUserPrompt(observationPackage) }
                    }
                }
            },
    ...
    /// <summary>
    /// Extracts concatenated text from Gemini generateContent JSON (candidates[].content.parts[].text).
    /// </summary>
    private static bool TryExtractGeminiTextContent(
        string responseBody,
        out string content,
        out string? failureReason)
    {
        ...
        using (document)
        {
            ...
            {
                var br = blockReason.GetString();
                if (!string.IsNullOrWhiteSpace(br))
                {
                    failureReason = $"blocked-prompt:{br}";
                    return false;
                }
            }
            ...
            {
                failureReason = "missing-candidates";
                return false;
            }
            ...
            {
                failureReason = "missing-content-parts";
                return false;
            }
